...
export class PixiBridge {
  ...
  private drawPath(graphics: PIXI.Graphics, pathData: string) {
    ...
    for (const [id, node] of Object.entries(nodes)) {
      ...
      const isNew = !pixiNode;

      if (!isNew && !node.isDirty && !node.appearanceDirty) {
        continue;
      }
